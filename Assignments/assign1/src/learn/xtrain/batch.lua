...
local batch = torch.class("xtrain.batch")
...
function batch:__init(model, loss, regu, decfunc, errfunc, optalg, state, config)
   self.model = model
   self.loss = loss
   self.regu = regu
...
function batch:train(dataset, epoch)
   ...
   for t = 1, epoch do
	 ...
	 for i = 1, dataset:size() do
	    ...
	    loss = loss + self.loss:forward(output, dataset[i][2])
	 ...
	 return (loss+r)/dataset:size(), dw:add(dr):div(dataset:size())
      end
